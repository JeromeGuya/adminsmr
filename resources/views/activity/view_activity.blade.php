        ...
        <div class="container mx-auto px-4">
            ...
            <div class="overflow-x-auto bg-white rounded-lg shadow dark:bg-gray-800">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                    @forelse ($activity as $activities)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                {{ $activities->activity_name }}
                            </td>
                            <td class="px-6 py-4">
                                {{ Str::limit($activities->activity_description, 50, '...') }}
                            </td>
                            <td class="px-6 py-4">
                                PHP{{ number_format($activities->rate, 2) }}
                            </td>
                            <td class="px-6 py-4">
                                @if($activities->availability)
                                    <span
                            ...
                            <td class="px-6 py-4 flex space-x-4">
                                <a href="{{ route('activity.edit', $activities->activity_id) }}"
                                ...
                                <button type="button" class="text-red-600 hover:underline dark:text-red-400"
                                        data-modal-toggle="deleteActivityModal-{{ $activities->activity_id }}"
                                        data-modal-target="deleteActivityModal-{{ $activities->activity_id }}">
                                    Delete
                                </button>

                                <!-- Modal for Delete Confirmation -->
                                <div id="deleteActivityModal-{{ $activities->activity_id }}" tabindex="-1"
                         ...
                         dark:text-gray-200" fill="none" stroke="currentColor"
                              ...
                              0 11-18 0 9 9 0 0118 0z"></path>
                                                ...
                                                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                                    Are you sure you want to delete {{ $activities->activity_name }}? This
                                                    action cannot be undone.
                                                </h3>
                                                <form action="{{ route('activity.destroy', $activities->activity_id) }}"
