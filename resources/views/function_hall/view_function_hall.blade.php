<x-layout.default>
    ...
    <div class="panel">
        <div class="container mx-auto px-4">
            ...
            <div class="overflow-x-auto bg-white rounded-lg shadow dark:bg-gray-800">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tr>
                        <th scope="col" class="px-6 py-3">Function Hall Name</th>
                    ...
                    <tbody>
                    @forelse ($hall as $halls)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                                <span class="block">{{ $halls->hall_name }}</span>
                                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $halls->hall_type }}</span>
                            </td>
                            <td class="px-6 py-4">
                                {{ Str::limit($halls->description, 50, '...') }}
                            </td>
                            <td class="px-6 py-4">
                                PHP{{ number_format($halls->rate, 2) }}
                            </td>
                            <td class="px-6 py-4">
                                @if($halls->availability)
                                    <span
                            ...
                            <td class="px-6 py-4 flex space-x-4">
                                <a href="{{ route('hall.edit', $halls->hall_id) }}"
                                ...
                                <button type="button" class="text-red-600 hover:underline dark:text-red-400"
                                        data-modal-toggle="deleteRoomModal-{{ $halls->hall_id }}"
                                        data-modal-target="deleteRoomModal-{{ $halls->hall_id }}">
                                    Delete
                                </button>

                                <!-- Modal for Delete Confirmation -->
                                <div id="deleteRoomModal-{{ $halls->hall_id }}" tabindex="-1"
                                        ...
                                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-800">
                                            ...
                                            <div class="p-6 text-center">
                                                ...
                                                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">
                                                    Are you sure you want to delete {{ $halls->hall_name }}? This action
                                                    cannot be undone.
                                                </h3>
        ...
        <!-- Pagination -->
        <div class="mt-6">
            {{ $hall->links('pagination::tailwind') }}
        </div>
    </div>
